...
/**
 * @file cliffordcore/cl2/rotor.hpp
 ...
 * explicit, because narrowing silently discards grade 1.
 *
 * In Cl(2,0) the even subalgebra is exactly the complex numbers: a rotor is
 * a + b e12 with e12^2 = -1, and rotor multiplication IS complex
 * multiplication. The alias Complex<T> names the same type, for code that
 * wants to say so.
 *
 * @author Filip Sramek
 * @version 0.3.0
...
namespace CliffordCore::Cl2
{
    template<typename T>
    /**
     * @brief A class representing a rotor in 2D space.
    ...
    public:
        ...
        /**
         * @brief Constructor initializes the rotor from a 2D multivector.
         * @param m The multivector to narrow.
         *
         * EXPLICIT on purpose: this discards the vector part of m. In Cl(3,0)
         * the same constructor was once implicit, and `rotor + multivector`
         * silently resolved to it and threw grades away with no diagnostic,
    ...
    template<typename T>
    /**
     * @brief The complex numbers, which are exactly the even subalgebra of Cl(2,0).
     *
     * A rotor is a + b e12 with e12^2 = -1, so it is a complex number in every
     * respect: rotor multiplication is complex multiplication, reverse() is
     * conjugation, norm() is the modulus and exp()/log() are the complex ones.
     * This alias exists so code that is thinking in complex numbers can say so.
     *
     * @tparam T The arithmetic component type.
     */
    using Complex = Rotor<T>;
